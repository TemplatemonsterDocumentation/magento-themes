<h3>Category listing settings</h3>

<p>The template includes two product listing layouts on the category page.</p> In order to select the required one:
<ol class="index-list"> 
    <li>
        In the top navigation menu go to <strong>System > Configuration</strong>. 
        <figure class="img-polaroid"><img src="img/magento/Theme-set-8.jpg" alt="" /></figure>
    </li>
    <li>
        Go to "<strong>TM Store Settings</strong>". In the "<strong>Product Listing Layout</strong>" section select the layout you like.
        <figure class="img-polaroid"><img src="img/magento/category-settings.jpg" alt="" /></figure>        
    </li>
    <li>
        Click <strong>"Save Config"</strong>.
    </li>
</ol>
<div class="alert alert-warning">
    <p>This option allows to change listing layout only on the product preview in grid view. List view layout has no changes.</p>
</div>
<p>
    In order to add banner to the category page in the top navigation menu go to <strong>Catalog > Manage Categories</strong>. Select the required category and in the <strong>Image</strong> field choose banner image.
</p>
<figure class="img-polaroid"><img src="img/magento/category-settings-1.jpg" alt="" /></figure>